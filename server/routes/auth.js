...
router.post('/login', [
    ...
    try {
        ...
        if (theUser) {
            ...
            if (checkHash) {
                let payload = { user: { id: theUser.id } };
                const authtoken = jwt.sign(payload, JWT_SECRET);
...
// Route 3: Update name via headers
router.put('/update', async (req, res) => {
    try {
        const { name } = req.body;
        const email = req.headers.email; // We use the email from the header

        if (!email) {
            return res.status(400).json({ error: "Email not found in headers" });
        }

        // CORRECTED: Search by email instead of username
...
// Route 5: Update specific user details
router.put('/user', [
    body('name', "Username should be at least 4 characters").isLength({ min: 4 }),
    body('phone', "Phone number should be 10 digits").isLength({ min: 10 }),
], async (req, res) => {
